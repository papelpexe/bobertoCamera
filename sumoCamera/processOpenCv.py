...
def seguidor_centro(img):
    ...
    if len(contours) > 0:
        ...
        if M['m00'] != 0:
            cx = int(M['m10']/M['m00'])
            error = (cx - widthbin // 2) * 640 // cam.camWidth
...
def verificaIntercessao(img):
    ...
    cam.frameProcIntercessao = visualizacao
...
def checarVerdes(img):
    ...
    valorAltoPreto = np.array([180, 55, 47])
    ...
    for i, cnt in enumerate(contoursVerde):
        ...
        if area > 100:
            ...
            indQuadrado = int(cam.camWidth / 8)
            ...
            if (bools[2] == False and bools[3] == True) or (bools[2] == True and bools[3] == False) or (bools[0] == True and bools[1] == True): 
                # if int(fheight * 1/10) < meio < int(fheight * 7/10):
                if int(fheight * 7/10) > meio > int(fheight * 1/10):
            ...
            cv2.putText(frame_com_contornos, verde_mov, (x, y+h+20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            cv2.line(frame_com_contornos, (0, int(meio)), (fwidth, int(meio)), (0,255,0), 2)
    ...
    cv2.line(frame_com_contornos, (0, int(fheight * 7/10)), (fwidth, int(fheight * 7/10)), (0,0,0), 2)
    cv2.line(frame_com_contornos, (0, int(fheight * 1/10)), (fwidth, int(fheight * 1/10)), (0,0,0), 2)

    cam.frameProcVerde = frame_com_contornos
